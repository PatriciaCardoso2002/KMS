...
int main() {
    ...
    auto outputSignalTX = std::make_shared<HandlerMessage>();
    ...
    SaveAscii saveAscii({outputSignalRX.get()}, {});
    ...
    while (true) {
        if (outputSignalLoadAscii->space() == 0 || !loadAscii.runBlock()) {
            ...
            messageHandlerRX.runBlock();
            i++;
            printf("i = %d\n", i);
            saveAscii.runBlock(); // Save processed data to file
